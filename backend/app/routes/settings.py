...
def get_my_appearance(current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not row:
        return AppearanceSetting(user_role=current_user["role"], user_id=current_user["id"], mode="day", accent="blue", font="default", skin="clean", updated_at=datetime.now().isoformat())
...
def get_appearance(user_role: str, user_id: str, db: Session = Depends(get_db)):
    ...
    if not row:
        return AppearanceSetting(user_role=user_role, user_id=user_id, mode="day", accent="blue", font="default", skin="clean", updated_at=datetime.now().isoformat())
