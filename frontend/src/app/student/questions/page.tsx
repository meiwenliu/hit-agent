...
export default function StudentQuestionHistoryPage() {
  ...
  if (!user || user.role !== "student") return <main className="section-card rounded-[28px] p-8 text-center text-slate-500">正在加载学习问答记录...</main>;
  ...
  return (
    <main className="glass-panel rounded-[32px] px-6 py-8 md:px-8">
      ...
      <div className="mt-6 space-y-4">
        {items.length === 0 ? <div className="section-card rounded-[28px] p-8 text-center text-slate-500">还没有历史提问，先去课程专属 AI 助教页面开始一次问答吧。</div> : items.map((item) => (
          <div key={item.id} className="section-card rounded-[28px] p-5">
            ...
            {item.ai_answer_content ? <div className="mt-4 rounded-[22px] bg-slate-900 px-5 py-4 text-sm leading-7 text-white"><p className="text-xs font-semibold text-slate-300">AI 回答</p><p className="mt-2 whitespace-pre-wrap">{item.ai_answer_content}</p></div> : null}
            <div className="mt-4 rounded-[22px] border border-slate-200 bg-white px-5 py-4 text-sm leading-7 text-slate-700"><p className="text-xs font-semibold text-slate-500">教师回答</p><p className="mt-2 whitespace-pre-wrap">{item.teacher_answer_content || "当前还没有教师补充回复。"}</p></div>
          </div>
        ))}
      </div>
    </main>
